...
또한 access 함수는 전역변수 hits, misses, evictions를 처리하고, "hit", "miss", "miss eviction" 중 하나의 문자열을 반환한다. "miss eviction"인 이유는 eviction이 일어났다면 항상 miss가 일어나기 때문이다.
...
graph LR
    ...
    D --> E{op is 'ILSM' ;}
    E --> |<p class='linename'>false</p>| X[Exit the Program ;]
    E --> |<p class='linename'>true</p>| F{argcnt is 3 ;}
    F --> |<p class='linename'>false</p>| X
    F --> |<p class='linename'>true</p>| G[Cache Access ;]
    G --> B
```
...
trace file의 각 줄은 위와 같은 format을 가진다. 이때 한 줄이 가질 수 있는 최대 크기를 어림해서 계산해보면, operation에 1 byte, hex_addr에 8 * 2 = 16 byte, decimal_size에 20 byte이기 때문에 약 40 byte이다. 따라서 이보다 큰 길이의 buffer를 주어 fgets 함수로 한 줄 씩 읽어올 수 있다.
...
마찬가지로, 전체 크기가 4096 바이트이기 때문에 OS 환경마다 다르지만 행렬 B[0]가 A[0]과 같은 set index를 받을 확률이 높다. 따라서 A와 B의 경우 같은 index를 참조할 경우 상호 배척 관계가 될 확률이 높다.
...
9. `b_bl[0]`에 `b_tr[0]`의 값을 저장한 register의 값을 저장한다. (miss `b_bl[0]`, possibly miss `a_bl[0]`)
  1. 이때 값은 `b_tr`에 저장될 때 이미 transpose된 상태이므로 그냥 저장해도 무방하다.
...
따라서 이 경우에는 일반적인 transpose를 수행해주면 된다. 물론 여전히 블록 단위로 충돌하진 않지만 같은 행렬 내에서 충돌하긴 하기 때문에 바로 transpose를 하면 안 되고 이것도 블록 단위로 나누어서 transpose를 해 줘야 한다. 다만 내부 알고리즘은 블록 단위 원소 transpose면 충분하다. 그 이유는 여전히 행렬의 서로 다른 줄이 충돌할 확률이 적기 때문이다.
...
다만 61x67의 경우에는 블록을 8x8로 제한할 필요는 없다. 이 역시 어떤 사이즈를 가지고 와도 블록 간의 충돌 가능성이 낮기 때문이다. 수학적으로 계산할 수는 있겠지만, 이 경우 무조건 블록의 크기는 32 이하여야 하기 때문에 직접 실험해서 측정할 수 있다.
...
[[컴퓨터 구조] 11. 캐시 성능 측정 및 향상 (https://narakit.tistory.com/138)](https://narakit.tistory.com/138)

[Performance x64: Cache Blocking (Matrix Blocking) (https://www.youtube.com/watch?v=G92BCtfTwOE)](https://www.youtube.com/watch?v=G92BCtfTwOE)

<div class="break"></div>

## Appendix

- $va_b$는 블록 순회 루프에서 b번째 iteration에 a번째 register(변수)를 의미한다.
- $va_{bc}$는 블록 순회 루프에서 b번째 for문의 c번째 iteration의 a번째 register(변수)를 의미한다.
- $+v$는 register에 해당 행렬 요소 부분의 값을 저장하는 것을 의미한다.
- $-v$는 register의 값을 해당 행렬 요소 부분에 저장하는 것을 의미한다.
- 각 저장 및 할당은 쓰인 순서대로 발생한다.
