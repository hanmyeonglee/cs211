...
[NX](#0-non-executable) 부분에서 설명했듯이 해당 스택은 custom stack으로 rwx 권한을 가지고 있기에 stack에 binary를 써넣으면 실행이 된다. 이때 rdi에 cookie의 값이 할당되는 코드가 실행되어야 한다. 따라서 solution은 다음과 같다.
