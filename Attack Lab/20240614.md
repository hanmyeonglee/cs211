...
`rtarget`은 [ctarget binary core structure](#binary-core-structure)와 다르게 `stable_launch` 부분이 없다. 이는 일반적인 stack을 사용하기에 실제로 사용할 스택은 NX라는 것을 뜻한다.

```
0x00007ffffff9f000 0x00007ffffffff000 0x0000000000000000 rw- [stack]
```

다만 `PIE`가 켜져있지 않으므로 ASLR이 적용되어도 프로그램 코드 주소는 바뀌지 않는다.
...
[touch 3](#6-touch-3)의 구조도 위에서 분석한 것과 같다. 그러나 가장 큰 문제는 주어진 buffer에 문자열을 그대로 써 넣을 수 없다는 것이다. 왜냐하면 rdi에 `$rsp - 0x38`의 값을 할당할 수 없기 때문이다. 이유는 우선 [Binary Information](#binary-information-1)에서 볼 수 있듯이 ASLR이 활성화되어 있기에 rsp의 값은 미리 써 넣을 수 없다. 따라서 rsp의 값을 직접 받아와야 하는데, 이때 이 값을 줄이는 방법이 존재하지 않는다. 따라서 buffer에 써 넣을 수는 없다.

그러나, rdi에 임의의 값을 더할 수는 있다. 왜냐하면 `lea rax, [rdi + rsi]; ret`이 있기 때문이다. rsi에는 임의의 값을 넣을 수 있는데, 이는 `pop rax; nop; ret`, `mov edx, eax; nop; ret`, `mov ecx, edx; and bl, bl; ret`, `mov esi, ecx; or dl, dl; ret`의 4개의 가젯을 이어서 stack에 써 넣은 임의의 값을 rsi에 넣을 수 있다. 또한 `mov rdi, rax; nop; ret`를 통해 계산한 rdi + rsi를 다시 rsi에 옮길 수 있으므로 rdi는 rsp 이상의 주소(4바이트 한정)에 항상 접근할 수 있다.

따라서, buffer를 초과해 임의의 길이의 padding을 주고 거기에 cookie hex string을 써 넣는다면 충분히 문자열 주소를 계산할 수 있다. solution은 다음과 같다.
