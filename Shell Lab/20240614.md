...
현재 `tsh.c`의 sigchld_handler에서는 SIO 함수를 사용한다. 그 이유는 printf가 Async Signal Safety 함수가 아니므로 handler에서는 사용하면 안 되기 떄문이다. printf는 내부적으로 Lock을 잡아서 함수 실행 중에는 다른 곳에서 비동기적으로 printf가 사용되더라도 현재 출력이 모두 출력되고 나서 다음 printf가 실행될 수 있게 안정적으로 동작하도록 짜여있다. 하지만 이 특징이 곧 Deadlock을 만든다.
