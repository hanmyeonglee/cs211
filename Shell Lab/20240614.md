...
그래서 handler에서는 Async Signal Safety한 출력 함수인 `write`를 사용하는 것이 옳다. 또는 제공되는 Safety I/O Library(SIO)가 있다면 그것을 사용해야만 한다. 하지만 이 과제에서 제공된 SIO가 없을 뿐더러 decimal formatting을 Async Safe하도록 재구성해서 write하는 것은 꽤 복잡하기 때문에 
